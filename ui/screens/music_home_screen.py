...
class MusicHomeScreen:
    ...
    def get_collection_card_surface(
        self,
        collection_key,
        collection,
        card_size
        ...
        song_keys = tuple(
            getattr(song, "audio_path", song.title)
            for song in songs[:3]
        )
        ...
        if cache_key in self.collection_card_cache:
            return self.collection_card_cache[
                cache_key
            ]
        ...
        count_surface = (
            self.playlist_card_meta_font.render(
                count_text,
                True,
                (125, 125, 135)
            )
        ...
        fitted_title = self.music.fit_text(
            ...
            max(20, title_max_width)
        )

        title_surface = (
            self.playlist_card_title_font.render(
                fitted_title,
                True,
                (205, 205, 215)
            )
        ...
        for index, song in enumerate(preview_songs):
            ...
            song_surface = (
                self.playlist_song_font.render(
                    song_text,
                    True,
                    (155, 155, 165)
                )
    ...
    def handle_home_key(self, key):
        ...
        if self.home_section == "playlists":
            ...
            if key == pygame.K_DOWN:
                ...
                if artists:
                    self.artist_card_index = max(
                        0,
                        min(
                            self.artist_card_index,
                            len(artists) - 1
                        )
                    )
        ...
        if self.home_section == "artists":
            ...
            if key == pygame.K_LEFT and artists:
                self.artist_card_index = max(
                    0,
                    self.artist_card_index - 1
                )
                return True

            if key == pygame.K_RIGHT and artists:
                self.artist_card_index = min(
                    len(artists) - 1,
                    self.artist_card_index + 1
                )
                return True

            if key == pygame.K_RETURN and artists:
                artist_name, _ = artists[
                    self.artist_card_index
                ]

                self.music.active_playlist_tag = (
                    "__artist__:" + artist_name
                )
    ...
    def home_playlists(self):
        playlists = [
            (
                "__all_songs__",
                {
                    "display_name": "All Songs",
                    "songs": list(self.music.library.songs),
                }
            ),
            (
                "__favorites__",
    ...
    def get_active_playlist(self):
        active_key = self.music.active_playlist_tag

        if active_key == "__all_songs__":
            return {
                "display_name": "All Songs",
                "songs": list(self.music.library.songs),
            }

        if active_key == "__favorites__":
            return {
                "display_name": "Favorites",
                "songs": self.favorite_songs(),
            }

        artist_prefix = "__artist__:"

        if (
            isinstance(active_key, str)
            and active_key.startswith(artist_prefix)
        ):
            artist_name = active_key[
                len(artist_prefix):
            ]

            return self.music.library.artists.get(
                artist_name
            )
    ...
    def update_scroll_target(self):
        ...
        viewport_top = int(h * 0.12)
        viewport_bottom = int(h * 0.92)
        ...
        if self.home_section == "playlists":
            selected_top = int(h * 0.525)
            selected_bottom = (
                int(h * 0.575)
                + card_height
            )

        elif self.home_section == "artists":
            selected_top = int(h * 0.925)
            selected_bottom = (
                int(h * 0.975)
                + card_height
            )
        ...
        if selected_bottom + target > viewport_bottom:
            target = viewport_bottom - selected_bottom

        if selected_top + target < viewport_top:
            target = viewport_top - selected_top
        ...
        self.scroll_target_y = float(target)

    def draw_collection_cards(
        self,
        screen,
        collections,
        selected_index,
        ...
        card_gap = int(self.music.width * 0.015)
        ...
        max_start = max(
            0,
            len(collections) - visible_card_count
        )
        ...
        for visible_index, (
            collection_key,
            collection
        ) in enumerate(visible_collections):
            ...
            selected = (
                self.home_section == section_name
                and actual_index == selected_index
            )

            card_surface = self.get_collection_card_surface(
                collection_key,
                collection,
                (
                    card_rect.width,
    ...
    def draw_home(self, screen):
        ...
        heading_y = int(h * 0.185) + page_offset_y
        ...
        card_top = int(h * 0.225) + page_offset_y
        ...
        cards_top = int(h * 0.575) + page_offset_y
        cards_width = int(w * 0.84)
        card_height = int(h * 0.285)

        self.draw_collection_cards(
            screen=screen,
            collections=playlists,
            selected_index=self.playlist_card_index,
        ...
        artists_heading = self.continue_heading_font.render(
            "Artists",
            True,
            (
                (245, 245, 250)
        ...
        self.draw_collection_cards(
            screen=screen,
            collections=artists,
            selected_index=self.artist_card_index,
            section_name="artists",
            cards_top=artists_cards_top,
            section_left=section_left,
            cards_width=cards_width,
            card_height=card_height
        )
